...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('autoapi.runAnalysis', runAnalysis), vscode.commands.registerCommand('sentinel.applyOpenApiQuickFix', applyOpenApiQuickFix), vscode.workspace.onDidSaveTextDocument((document) => {
    ...
    }), vscode.languages.registerCodeActionsProvider(SUPPORTED_SOURCE_LANGUAGES, new SentinelCodeActionProvider(), { providedCodeActionKinds: [vscode.CodeActionKind.QuickFix] }));
...
function scheduleLiveLint(document) {
    ...
    const delay = vscode.workspace.getConfiguration('sentinel').get('liveLint.debounceMs', 650);
...
async function runLiveLint(document) {
    if (!isSupportedSourceDocument(document) || !liveLintEnabled()) {
        return;
    }
    const rootPath = workspaceRoot();
...
function toVsCodeDiagnostic(document, item, specPath) {
...
class SentinelCodeActionProvider {
    provideCodeActions(document, _range, context) {
        ...
        for (const diagnostic of context.diagnostics) {
            ...
            const action = new vscode.CodeAction('Update OpenAPI spec to match this code', vscode.CodeActionKind.QuickFix);
...
async function applyOpenApiQuickFix(documentUri, payload) {
    const rootPath = workspaceRoot();
    if (!rootPath) {
        vscode.window.showErrorMessage('Must be inside a workspace.');
        return;
...
function runCli(rootPath, args) {
    const pythonPath = vscode.workspace.getConfiguration('sentinel').get('pythonPath', 'python');
    const cliPath = path.join(rootPath, 'cli', 'main.py');
    return new Promise((resolve) => {
        (0, child_process_1.execFile)(pythonPath, [cliPath, ...args], { cwd: rootPath }, (error, stdout, stderr) => {
            resolve({
                stdout,
                stderr,
...
async function findSpecPath(rootPath) {
    const configured = vscode.workspace.getConfiguration('sentinel').get('specPath', '');
    if (configured) {
...
function liveLintEnabled() {
    return vscode.workspace.getConfiguration('sentinel').get('liveLint.enabled', true);
}
